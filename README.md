...
- Anonymous types will be implemented the same way as C# dynamics (using the CSharp RuntimeBinder stuff) _except_ the calling context will be the anonymous type itself
    ...
    - __The implications:__
        ...
        - Since DLR is actually kind of performant, dynamics and anonymous types will be almost, if not, as performant here as they are on hxnodejs (or at least they should be if DLR doesn't suck, but there are great similarities between the approach it takes and the approach v8 takes for runtime caching/code generation based on callsites and runtime type inspection/reflection)
