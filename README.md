...
## Update 10/7/2021 Closures/anonymous functions
Closures and anonymous functions roughly work at least for instance methods... in fact static calls don't work at all right now, hence, there are no calls to any logging methods at the moment. Given that fact, you may consider this the world's longest no-op _/s_
![](https://github.com/piboistudios/hxgenpe/blob/master/worlds-longest-noop.PNG?raw=true)
...
# Other Notes:
- __LibIlasm__ refers to LibIlasm.dll (see the native folder). Right now, this is just Mono ILASM converted into a class library as opposed to an executable assembly. This uses LibIlasm, but it does not generate CIL (nor does it actually use the ILASM application itself).
